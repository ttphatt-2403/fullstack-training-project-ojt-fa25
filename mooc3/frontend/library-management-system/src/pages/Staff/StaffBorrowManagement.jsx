...
function StaffBorrowManagement() {
  ...
  const fetchOverdueBorrows = async (page = 1, pageSize = 20) => {
    ...
    try {
      ...
      // Lọc những record quá hạn (dueDate < ngày hiện tại)
      const today = new Date();
  ...
  const loadUsersAndBooks = async () => {
    try {
      ...
      setBooks((booksResponse.items || []).filter(book => (book.availableCopies || 0) > 0));
    } catch (error) {
  ...
  const handleRejectBorrow = async (values) => {
    try {
      ...
      await borrowService.rejectBorrowRequest(borrowId, { Notes: values.notes });
  ...
  const openCreateFeeModal = () => {
    ...
    if (borrowDetail.status === 'borrowed' && borrowDetail.daysOverdue > 0) {
      suggestedAmount = borrowDetail.daysOverdue * 5000; // 5000 VND per day
    ...
    },
    {
      ...
      render: (days) => (
        <span style={{ color: 'red', fontWeight: 'bold' }}>
          {(days * 5000).toLocaleString('vi-VN')}đ
        </span>
      ),
      width: '12%'
    },
              ...
              {borrowDetail.daysOverdue !== null && borrowDetail.daysOverdue > 0 && (
                <Descriptions.Item label="Số ngày trễ" span={2}>
                  ...
                  <span style={{ marginLeft: 8, color: 'red', fontWeight: 'bold' }}>
                    (Phí phạt: {(borrowDetail.daysOverdue * 5000).toLocaleString('vi-VN')}đ)
                  </span>
                </Descriptions.Item>
              )}
